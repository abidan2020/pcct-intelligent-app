...
# =========================
    ...
    with col2:
        heures = st.number_input("Heures d’utilisation du scanner", min_value=0, max_value=100000, value=0)
        bruit = st.slider("Niveau de bruit image (%)", 0, 100, 0)
    ...
    df_maintenance = pd.DataFrame({
        "Paramètre": [
            "Scanner", "SNR système", "Température tube RX", "Vibration gantry",
            "Bruit image", "Heures d’utilisation", "État détecteurs",
            "Refroidissement", "Score stress", "État global", "Composant suspect"
        ],
        "Valeur": [
            scanner, snr_sys, f"{temperature} °C", f"{vibration} %",
            f"{bruit} %", heures, detecteurs, refroidissement,
            f"{score} %", etat, composant
        ]
    })
